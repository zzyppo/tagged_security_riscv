...
abstract trait MemSpaceParameters extends UsesParameters {
  ...
  val nTagCacheTrackers = params(TCTrackers)        // the number of concurrent trackers
  require(xLen >= pALen) // TODO: support pALen > xLen

}


class AddrConvReq extends Bundle with MemSpaceParameters
{
  //val ch = UInt(width = 2);
  val id = UInt(width  = log2Up(nTagCacheTrackers + 1))
  val core_addr = UInt(width = pALen)
}

class AddrConvResp extends Bundle with MemSpaceParameters
{
//  val ch = UInt(width = 2);
  val id = UInt( width  = log2Up(nTagCacheTrackers + 1))
  val phy_addr = UInt(width = pALen)
}

class AddressConvIO extends Bundle {
  val req = Decoupled(new AddrConvReq)
  val resp = Valid(new AddrConvResp).flip
}

class MemSpaceConstsWithIO(ch: Int) extends Module with MemSpaceParameters {
  val io = new Bundle {
    val update = new ValidIO(new PCRUpdate).flip
    val addrConvIO = new AddressConvIO().flip
  }
  ...
  io.addrConvIO.req.ready := Bool(true)

  // address converter
  val addr_vec_new = Wire(Vec(UInt(width=pALen), nMemSections))
  for(i <- 0 until nMemSections) {
    addr_vec_new(i) :=
      Mux((io.addrConvIO.req.bits.core_addr.toBits() & ~ mask(i)(pALen,0)) === cbase(i)(pALen,0),
        io.addrConvIO.req.bits.core_addr.toBits() & mask(i) | pbase(i),
        UInt(0)
      )
  }

 /* when(io.addrConvIO.req.valid)
  {
  ...
  */
  ...
  io.addrConvIO.resp.valid := ShiftRegister(io.addrConvIO.req.fire(), 1)
  io.addrConvIO.resp.bits.id := ShiftRegister(io.addrConvIO.req.bits.id, 1)
  io.addrConvIO.resp.bits.phy_addr := addr_vec_new.reduce(_|_)
}
