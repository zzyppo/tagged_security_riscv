...
class Rocket (id:Int, resetSignal:Bool = null) extends CoreModule(resetSignal)
{
  ...
  val ex_reg_rs_lsb = Vec(Reg(init=UInt(0)), id_raddr.size)
  val ex_reg_rs_msb = Vec(Reg(init=UInt(0)), id_raddr.size)
  val ex_reg_tags_pre = Vec(Reg(init=UInt(0)), id_raddr.size)
  ...
  val ex_slow_bypass = ex_ctrl.mem_cmd === M_XSC || Vec(MT_B, MT_BU, MT_H, MT_HU, MT_T).contains(ex_ctrl.mem_type)
