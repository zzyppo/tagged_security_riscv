...
static void enter_entry_point()
{
   ...
   // write_csr(0x400, 0x7); //IO incalid tag generation and both checks on
    write_csr(0x400, 0xF); //Enable debug + IO incalid tag generation and both checks on
      //write_csr(0x400, 0xB); //Enable debug + IO incalid tag generation and both checks on
